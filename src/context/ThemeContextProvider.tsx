...
const ThemeContextDefaultProvider = (props: any) => {
  ...
  useEffect(() => {
    ...
    window
     ...
     const themeValue= localStorage.getItem('memoiristThemeValue');
     const colorScheme: ThemeMode = (themeValue === "dark" || themeValue === "light") 
                        ?  themeValue : userColorScheme;
                        
   localStorage.setItem('memoiristThemeValue', colorScheme);
  ...
  const toggleUserThemeMode = () => {
    ...
    localStorage.setItem('memoiristThemeValue', toggledThemeMode);
